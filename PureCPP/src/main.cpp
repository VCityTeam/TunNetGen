...
int main(int argc, char *argv[])
{
  ...
  int n_sample = 500;
  std::string doc_n_sample("Numbers of samples, in phi and in theta, for the "
                           "point cloud output.\n"
                           "The point cloud size is then NxN. Default is ");
  app.add_option("-N, --num_sample",
                 n_sample, doc_n_sample + std::to_string(n_sample) + ".");
  bool splitted = false;
  ...
  for (unsigned i = 0; i < poses.size(); ++i)
  {
    if (splitted)
    {
      out = outpath.stem().string() + "-" + std::to_string(i) + outpath.extension().string();
